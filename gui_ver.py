...
def judge_ymd(from_year_, from_month_, from_day_, to_year_, to_month_, to_day_):
    ...
    if int(from_year_) - int(to_year_) > 0 or int(from_year_) - int(to_year_) < 0 :
        ...
        error('その月はまだ入力できません')
    elif (int(from_month) == int(to_month) and int(from_day_) - int(to_day_) >= 0) or (int(from_month) - int(to_month) < 0 and int(from_day_) - int(to_day_) >= 0):
...
def create_gui():
    ...
    write_label(day_txt  + '曜日です', "MSゴシック", "12", 330, 105)
