...
def get_ymd():
    ...
    return from_year, from_month, from_day, to_year, to_month, to_day, holiday, isKarihozon_, auto_start_
...
def karihozon_check():
    ...
    chk_yes_karihozon.place(x=70, y=295)
...
def get_combobox(text_, num):
    ...
    if date_now.month < from_month or date_now.month + 1 < to_month:
        error('その月はまだ入力できません')
        count -= 1
    elif (date_now.month == from_month and date_now.day < from_day) or (date_now.month == to_month and date_now.day < to_day):
        error('その日はまだ入力できません')
        count -= 1
...
def entry(txt_, list_, width_, x_, y_):
    ...
    if txt_ == 'from_year':
        ...
        holiday_combobox = ttk.Combobox(width= width_, height = len(list_), justify="center", values = list_, textvariable = variable)
        holiday_combobox.place(x=x_, y=y_)
        holiday_combobox.bind('<<ComboboxSelected>>', lambda e: get_combobox(txt_, variable.get()))
...
def create_gui():
    ...
    # ※記載
    write_label('※ウィンドウは動かさないことを推奨します。', "MSゴシック", "9", 120, 370)
    ...
    # 年数入力 ～から
    entry('from_year', year, 4, 155, 150)
    # ～まで
    entry('to_year', year, 4, 155, 200)
    # 年ラベル ～から
    ...
    for i in range(12):
        month.append(i+1)
    # 月入力 ～から
    entry('from_month', month, 4, 235, 150)
    # ～まで
    entry('to_month', month, 4, 235, 200)
    # 月ラベル  ～から
    ...
    for i in range(31):
        day.append(i+1)
    # 日入力 ～から
    entry('from_day', day, 4, 310, 150)
    # ～まで
    entry('to_day', day, 4, 310, 200)
    # 日ラベル ～から
    ...
    # 今週祝日があった場合の日付入力
    # 5日分 今週の月曜日から金曜日までの日付
    holiday = []
    for i in range(5):
        holiday.append(date_now.day + i)
    write_label('今週祝日があった場合祝日の日にちを入力してください', "MSゴシック", "9", 100, 230)
    entry('holiday', holiday, 4, 220, 260)
    write_label('日', "MSゴシック", "9", 280, 260)
    ...
    # 開始ボタン
    push_button("開始", "交通費自動化申請を開始しますか?", 20, 300, 295)
    #ウィンドウ閉じるボタン
    push_button("終了", "GUIウィンドウを閉じてもよろしいですか？", 20, 300, 335)
