...
from_year = monday.year
from_month = monday.month
from_day = monday.day
to_year = friday.year
to_month = friday.month
to_day = friday.day
# 初期は仮保存しない
isKarihozon_ = 0
auto_start_ = False
...
def judge_ymd(from_year_, from_month_, from_day_, to_year_, to_month_, to_day_):
    ...
    if date_now.year < from_year_ or date_now.year < to_year_:
        error('その年はまだ入力できません')
    elif from_month_ - to_month_ > 0:
        error('その月はまだ入力できません')
    elif from_day_ - to_day_ > 0:
...
def get_ymd():
    ...
    return from_year, from_month, from_day, to_year, to_month, to_day, isKarihozon_, auto_start_
...
# 日付入力する関数
def entry(txt_, list_, width_, x_, y_, init_day_):

    variable = IntVar()
    variable.set(str(init_day_))
...
def create_gui():
    ...
    # 安川画像表示
    path=__file__
    name = path.rstrip('gui.py')+'yaskawa.png'
    ...
    # 年数入力 ～から
    entry('from_year', year, 4, 155, 150, int(monday.year))
    # ～まで
    entry('to_year', year, 4, 155, 200, int(friday.year))
    # 年ラベル ～から
    ...
    for i in range(12):
        month.append(i+1)
    # 月入力 ～から
    entry('from_month', month, 4, 235, 150, int(monday.month))
    # ～まで
    entry('to_month', month, 4, 235, 200, int(friday.month))
    # 月ラベル  ～から
    ...
    for i in range(31):
        day.append(i+1)
    # 日入力 ～から
    entry('from_day', day, 4, 310, 150, int(monday.day))
    # ～まで
    entry('to_day', day, 4, 310, 200, int(friday.day))
    # 日ラベル ～から
